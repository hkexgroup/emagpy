...
def getR0_1(lamb, sigg, f, d):
    ...
    N = u/(1j*mu_0*omega)
    Y = np.zeros(N.shape, dtype=np.complex64)*np.nan # Y is actually -1 smaller than N but easier like this for index
    Y[-1] = N[-1]
...
def getR0_2(lamb, sigg, f, d):
    ...
    R = np.zeros((len(sigma)), dtype=np.complex64)#*np.nan
...
#    gamma = np.sqrt(lamb**2 + 1j*2*np.pi*f*mu_0*sigma)
    R = np.zeros((sigmaLen, len(lamb)), dtype=np.complex64)#*np.nan
